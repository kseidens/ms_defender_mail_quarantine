# MS Defender Mail Quarantine - Splunk SOAR App
...
4. After installation, go to **Apps -> MS Defender Mail Quarantine -> Configure New Asset**.
...
|---|---|---|---|
| `function_base_url` | yes | - | Azure Function App API base URL, **no trailing slash**. Example: `https://contoso-quarantine.azurewebsites.net/api` |
| `function_key` | no | - | Azure Function key. Leave empty only if the Function auth level is `Anonymous` |
| `auth_method` | no | `header` | How to send the key: `header` (x-functions-key, recommended), `query` (?code=), or `none` |
| `verify_ssl` | no | `true` | Verify the Azure Function SSL certificate. Always `true` in production |
| `timeout` | no | `120` | HTTP timeout in seconds. PowerShell cold starts and EXO cmdlets are slow -- minimum 60 s recommended |
| `export_mode` | no | `vault` | `vault` (recommended) stores EML in SOAR Vault and returns `vault_id`; `inline` returns base64 EML directly |
...
To run: **Apps -> MS Defender Mail Quarantine -> your asset -> Test Connectivity**

A successful response logs:
```
Successfully connected to Azure Function (service: ms-defender-mail-quarantine)
```
...
|---|---|---|
| `identity` | yes | Quarantine identity in format `GUID1\GUID2` (from list action output) |
...
> **[!] Irreversible.** The message is delivered to all original recipients immediately.

**Parameters:**

| Name | Required | Description |
|---|---|---|
| `identity` | yes | Quarantine identity in format `GUID1\GUID2` |
...
1. Open the Azure Function App in the Azure Portal -> **Functions -> your function -> Function Keys**.
2. Copy a valid key.
...
| `ms_defender_mail_quarantine_v1.3.0.zip` | **Installable SOAR app package** -- upload this to SOAR |
